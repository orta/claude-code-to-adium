...
async function generateHTML(
  ...
  try {
    ...
    // Try multiple locations: current directory, script directory, and parent directory
    const possibleIconPaths = [
      path.join(process.cwd(), "claude-icon-filled-256.png"),
      path.join(__dirname, "claude-icon-filled-256.png"),
      path.join(__dirname, "..", "claude-icon-filled-256.png"),
    ];
    ...
    for (const claudeIconSource of possibleIconPaths) {
    ...
    if (!iconCopied) {
      console.warn(
        "Warning: claude-icon-filled-256.png not found. Claude messages may not display an icon."
      );
    }
    ...
    if (commandArgs.length > 0) {
      commandUsed = `${commandPrefix} ${commandArgs.join(" ")}`;
    } else {
      // If interactive mode, show the equivalent command
      commandUsed = `${commandPrefix} "${projectName}" "${conversationId}" "${themeName}"`;
    }

    // Generate HTML without base href, using local references
    let htmlContent = `
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
<!-- Generated by: ${commandUsed} -->
<html><head>
